...
public class BrokerUnitTest {
    ...
    @Test
    public void test3_getQuotations_constantAuld() throws Exception {
        // unbound registry
        try {
            UnicastRemoteObject.unexportObject(registry, true);
        } catch (Exception e) {
            System.out.println("Trouble in getQuotations_constantAuld" + e);
        }
        // create new registry
        registry  = LocateRegistry.createRegistry(4099);
        LocalBrokerService localBrokerService = new LocalBrokerService(registry);
        BrokerService brokerService = (BrokerService) UnicastRemoteObject.exportObject(localBrokerService,0);
        registry.bind(Constants.AULD_FELLAS_SERVICE, brokerService);
        ClientInfo clientInfo = new ClientInfo("Niki Collier", ClientInfo.FEMALE, 49, 1.5494, 80, false, false);
        BrokerService service = (BrokerService) registry.lookup(Constants.AULD_FELLAS_SERVICE);
        List<Quotation> quotations = service.getQuotations(clientInfo);
        assertNull(quotations);
    }
}
